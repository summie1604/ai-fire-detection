...
BAUD_RATE = 9600
FLAME_THRESHOLD = 400  # 🔧 Adjusted after analysis
...
def predict(sensor_data):
    ...
    print(f"🔍 Model: {model_output:.2f}, MQ2: {sensor_data[0]}, MQ7: {sensor_data[1]}, Flame: {sensor_data[2]}")
...
try:
    while not fire_triggered:
        ...
        if line:
            try:
                sensor_data = list(map(int, line.split(", ")))[:3]
